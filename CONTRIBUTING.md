...
Wearable ingestion uses the native bridge's authenticated source, session and lease boundary. It stores complete aligned intervals through `WearableRepository`, with sample and time weighting kept separate. Test corrections and deletion after compaction, latest-batch retries, sequence conflicts, unload/reload and complete archive replacement. Keep the frozen cross-language batch and snapshot vectors unchanged. See [wearable history](docs/wearable-series.md). Unit fixtures exercise the receiver; only observed phone behavior qualifies an exporter.
