...
- [ ] Export all thirteen archive domains, including bridge ledgers and complete wearable snapshots. Import an actual format 1 archive from 0.2 and verify that existing sleep, recovery, bridge and wearable history is untouched. Default import is a dry run that leaves the destination unchanged. Apply it, check claims, exclusions, workouts and room history, then repeat without duplicates. Keep interrupted-import and conflicting-history regression results with the evidence.
...
- [ ] A synthetic recovery provider stores SDNN and RMSSD in separate series, normalizes supported units, preserves windows and unknown context, applies corrections and ordered deletions, and keeps local exclusions. Verify bounded source-series queries, admin-only exclusion, archive dry run/apply and unload/reload persistence on stable and minimum Home Assistant. Synthetic records do not qualify a vendor connection.
- [ ] A downloaded diagnostic contains counts and status without health values, titles, source payloads or credentials.
- [ ] Native bridge capture verifies current authentication, source ownership, corrections, deletions and pause/rearm after reload and restore. Record actual Apple Health or Health Connect producer evidence separately from synthetic protocol fixtures.
- [ ] Wearable capture stores and queries sample-weighted and time-weighted heart-rate intervals, replaces corrected intervals, preserves deletion through compaction and import, and rejects stale or conflicting sequences. Verify bounded pagination, complete snapshot replacement, backup restore and explicit rearm on stable and minimum Home Assistant. Document which phone route supplied the complete replacements and deletion evidence.
