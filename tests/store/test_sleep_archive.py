...
def test_full_roundtrip_uses_source_revision_and_exclusion_or(
    ...
    assert manifest["format_version"] == 2 and manifest["source_schema_version"] == 10
...
def test_forged_sleep_record_fails_before_any_live_write(database, target, tmp_path):
    ...
    payload = json.loads(dict(members)["sleep_sessions.jsonl"])
    ...
    members = [
        (name, data if name == "sleep_sessions.jsonl" else content)
